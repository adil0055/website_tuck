import React from "react";

export default function TuckSolutions() {
    return (
        <section className="bg-white" data-node-id="40:1304">
            {/* Two Column Layout */}
            <div className="grid lg:grid-cols-[1fr_2fr]">
                {/* Left Column - WHITE Background */}
                <div className="bg-white px-6 lg:px-[92px] py-16 lg:py-[76px] flex flex-col justify-start">
                    {/* Section Label */}
                    <p 
                        className="font-body text-[20px] font-semibold text-graphite mb-6 tracking-[-0.1px] leading-[61px]" 
                        data-node-id="40:1322"
                    >
                        Tuck solutions
                    </p>

                    {/* Main Heading */}
                    <h2 
                        className="font-heading text-[32px] sm:text-[40px] lg:text-[48px] font-extrabold text-graphite leading-[61px] tracking-[-0.1px]" 
                        data-node-id="40:1321"
                    >
                        Virtual Try‑Ons.<br />Obvious Fit.
                    </h2>
                </div>

                {/* Right Column - BLUE Background Split Horizontally */}
                <div className="bg-[#03c]" data-node-id="40:1323">
                    {/* Solution 01 - Tuck API */}
                    <div className="relative border-b border-gray-300 lg:border-b-0">
                        {/* Container for number and content */}
                        <div className="grid lg:grid-cols-[248px_1fr] min-h-[387px]">
                            {/* Number Badge Area - 248px wide */}
                            <div className="flex items-center justify-center py-12 lg:py-0">
                                <div 
                                    className="font-heading text-[40px] font-medium leading-[26px] text-white" 
                                    data-node-id="40:1327"
                                >
                                    01
                                </div>
                            </div>

                            {/* Content Area - White Background */}
                            <div className="bg-white px-6 lg:px-[92px] py-12 lg:py-[74.7px] flex flex-col justify-center">
                                {/* Tuck API Title */}
                                <h3 
                                    className="font-heading text-[28px] sm:text-[32px] font-semibold text-graphite mb-4 lg:mb-[6px] leading-[61px] tracking-[-0.1px]" 
                                    data-node-id="40:1329"
                                >
                                    Tuck API
                                </h3>

                                {/* Subtitle */}
                                <p 
                                    className="font-heading text-[15px] font-bold text-graphite mb-3 lg:mb-[4px] leading-[26px]" 
                                    data-node-id="40:1333"
                                >
                                    For ecommerce brands losing carts to size doubt
                                </p>

                                {/* Description */}
                                <p 
                                    className="font-body text-[17px] font-medium text-graphite mb-6 lg:mb-[24px] leading-[25.2px] max-w-[471px]" 
                                    data-node-id="40:1331"
                                >
                                    Tuck's virtual try-on plugin lets shoppers see outfits on themselves right on the product page, turning size doubt into higher conversions and fewer returns.
                                </p>

                                {/* Buttons */}
                                <div className="flex flex-wrap items-center gap-3 lg:gap-[29px]">
                                    <button 
                                        className="font-body font-semibold bg-graphite text-white px-6 py-2 text-[15px] hover:bg-opacity-90 transition-all h-[34px] flex items-center justify-center" 
                                        data-node-id="40:1337"
                                    >
                                        <span data-node-id="40:1339">Book a Demo</span>
                                    </button>
                                    <button 
                                        className="font-body font-medium text-graphite text-[15px] hover:underline" 
                                        data-node-id="40:1335"
                                    >
                                        Learn more
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>

                    {/* Horizontal Divider Line - Between Solutions */}
                    <div 
                        className="hidden lg:block w-full h-[1px] bg-black" 
                        data-node-id="40:1325"
                    />

                    {/* Solution 02 - Tuck Kiosk */}
                    <div className="relative">
                        {/* Container for number and content */}
                        <div className="grid lg:grid-cols-[248px_1fr] min-h-[387px]">
                            {/* Number Badge Area - 248px wide */}
                            <div className="flex items-center justify-center py-12 lg:py-0">
                                <div 
                                    className="font-heading text-[40px] font-medium leading-[26px] text-white" 
                                    data-node-id="40:1328"
                                >
                                    02
                                </div>
                            </div>

                            {/* Content Area - White Background */}
                            <div className="bg-white px-6 lg:px-[92px] py-12 lg:py-[74.7px] flex flex-col justify-center">
                                {/* Tuck Kiosk Title */}
                                <h3 
                                    className="font-heading text-[28px] sm:text-[32px] font-semibold text-graphite mb-4 lg:mb-[6px] leading-[61px] tracking-[-0.1px]" 
                                    data-node-id="40:1330"
                                >
                                    Tuck Kiosk
                                </h3>

                                {/* Subtitle */}
                                <p 
                                    className="font-heading text-[15px] font-bold text-graphite mb-3 lg:mb-[4px] leading-[26px]" 
                                    data-node-id="40:1334"
                                >
                                    For retailers losing inventory to wrong fits and style mismatches
                                </p>

                                {/* Description */}
                                <p 
                                    className="font-body text-[17px] font-medium text-graphite mb-6 lg:mb-[24px] leading-[25.2px] max-w-[471px]" 
                                    data-node-id="40:1332"
                                >
                                    Tuck's in-store kiosk helps customers find pieces that fit and suit them before purchase. Fewer returns, less dead stock, and higher confidence at checkout—turning inventory into revenue.
                                </p>

                                {/* Buttons */}
                                <div className="flex flex-wrap items-center gap-3 lg:gap-[29px]">
                                    <button 
                                        className="font-body font-semibold bg-graphite text-white px-6 py-2 text-[15px] hover:bg-opacity-90 transition-all h-[34px] flex items-center justify-center" 
                                        data-node-id="40:1338"
                                    >
                                        <span data-node-id="40:1340">Book a Demo</span>
                                    </button>
                                    <button 
                                        className="font-body font-medium text-graphite text-[15px] hover:underline" 
                                        data-node-id="40:1336"
                                    >
                                        Learn more
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    );
}
